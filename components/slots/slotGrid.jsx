      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <motion.button
            ...
            className={cn(
              "relative flex flex-col items-center justify-center gap-0.5 rounded-md border-2 h-14 transition-all duration-150 select-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70",
              typeConfig.width,
              statusStyle.border,
              statusStyle.bg,
              statusStyle.cursor,
          ...
          >
            ...
            {slot.status === SLOT_STATUS.BOOKED && (
              <Lock
                className="absolute top-0.5 right-0.5 w-3 h-3 text-red-500 opacity-80"
                aria-hidden
              />
            )}
            {slot.status === SLOT_STATUS.HELD && !isMyHold && (
              <Clock
                className="absolute top-0.5 right-0.5 w-3 h-3 text-amber-400 opacity-80"
                aria-hidden
              />
            )}
            {isSelected && (
              <CheckCircle2
                className="absolute top-0.5 right-0.5 w-3.5 h-3.5 text-white"
                aria-hidden
              />
            )}
...
export function SlotGrid({
  ...
  // Group by floor, then apply type filter
  ...
  const allTypes = [...new Set(Object.values(slots).map((s) => s.type))]

  if (error) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-16 text-slate-500">
        <Info className="w-8 h-8" />
        <p className="text-sm">Failed to load slots: {error}</p>
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="w-14 h-14 rounded-md border-2 border-slate-800 bg-slate-900 animate-pulse"
        />
      ))}
    </div>
  )
